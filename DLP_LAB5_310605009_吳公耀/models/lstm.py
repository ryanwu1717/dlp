...
class lstm(nn.Module):
    def __init__(self, input_size, output_size, hidden_size, n_layers, batch_size, device,conditional = True):
        ...
        if self.conditional:
            ...
            #hidden_size hame to time to because there are condition
    ...
    def forward(self, input, c=None):#c is the condition
    
        embedded = self.embed(input)
        embeddedcond = self.embedcond(c)
        embeddedboth = torch.cat((embedded, embeddedcond), dim=1) #concate input and condition make better result
        h_in = embeddedboth 
