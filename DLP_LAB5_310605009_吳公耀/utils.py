...
def kl_criterion4(mu1, logvar1, mu2, logvar2, args):
...
def plot_pred(seq,cond, modules, epoch, args, device):
    seq= seq.to(device)
    cond= cond.to(device)
    nsample = 1
    
    gt_seq = [seq[i] for i in range(len(seq))]
    ...
    for s in range(nsample):
        ...
        for i in range(1, args.n_past+args.n_future):
            ...
            if i < args.n_past:
                ...
                h = modules['frame_predictor'](torch.cat([h, z_t], 1),cond[i]).detach()
...
#for test to calculate psnr
def pred(validate_seq, validate_cond, modules, args, device,epoch):
    ...
    modules['frame_predictor'].hidden = modules['frame_predictor'].init_hidden()
    modules['posterior'].hidden = modules['posterior'].init_hidden()
    tmpseq = [modules['encoder'](validate_seq[0])] 
    x_in = validate_seq[0]
    for i in range(1,  args.n_past + args.n_future):
        ...
        h = h.detach()  
        if i < args.n_past:
            h_target = modules['encoder'](validate_seq[i])[0].detach()
