...
class Worker(mp.Process):
    def __init__(self, gnet, opt, global_ep, global_ep_r, res_queue, name , baseline ,args,start_training_time,time_stamp):
        super(Worker, self).__init__()
        self.name = 'w%i' % name
        self.g_ep, self.g_ep_r, self.res_queue = global_ep, global_ep_r, res_queue
        self.gnet, self.opt = gnet, opt
        self.baseline,self.args = baseline ,args
        self.start_training_time,self.time_stamp= start_training_time,time_stamp
        
        self.lnet = TSP_net(args.dim_input_nodes, args.dim_emb, args.dim_ff, 
              args.nb_layers_encoder, args.nb_layers_decoder, args.nb_heads, args.max_len_PE,
              batchnorm=args.batchnorm)           # local network

        self.lnet = self.lnet.to(device)
        
#         self.opt = SharedAdam(self.lnet.parameters(), lr=args.lr, betas=(0.92, 0.999))
#         if self.name != 'w0':
#             self.opt =  torch.optim.Adam(self.lnet.parameters(), lr=args.lr)
    def run(self):
        total_step = 0
        total_loss=0
        model_baseline = self.baseline
...
#             lnet = self.gnet
        ...
        while self.g_ep.value < self.args.nb_epochs:
#             print(self.g_ep.value)
            ...
            lnet.load_state_dict(self.gnet.state_dict())
            ...
            for step in range(1,args.nb_batch_per_epoch+1):
                ...
                tour_train, sumLogProbOfActions = self.lnet(x, deterministic=False) # size(tour_train)=(bsz, nb_nodes), size(sumLogProbOfActions)=(bsz)
                ...
                total_loss+=loss
                if (total_step%self.args.UPDATE_GLOBAL_ITER == 0):
                    
                    loss = total_loss/self.args.UPDATE_GLOBAL_ITER
                    self.opt.zero_grad()
                    loss.backward()

    #                 if self.name != 'w0':
                    for lp, gp in zip(lnet.parameters(), self.gnet.parameters()):
                        gp._grad = lp.grad.cuda()
            ...
            for step in range(0,args.nb_batch_eval):
                ...
                with torch.no_grad():
                    tour_train, _ = lnet(x, deterministic=True)
                    tour_baseline, _ = model_baseline(x, deterministic=True)
            ...
            with self.g_ep.get_lock():
                self.g_ep.value += 1
                if(self.g_ep.value > 0) :
                    for param_group in self.opt.param_groups:
                        param_group['lr'] = param_group['lr'] *  0.9
                        
                with self.g_ep_r.get_lock():
                    if self.g_ep_r.value == 0.:
                        self.g_ep_r.value = mean_tour_length_train
                    else :
                        self.g_ep_r.value = self.g_ep_r.value * 0.99 + mean_tour_length_train * 0.01
                self.res_queue.put(mean_tour_length_train)
            
                # evaluate train model and baseline and update if train model is better
                update_baseline = mean_tour_length_train+args.tol < mean_tour_length_baseline
                if update_baseline:
                    model_baseline.load_state_dict( self.gnet.state_dict() )
...
#                 plot_performance_baseline.append([ (epoch+1), mean_tour_length_baseline])
                ...
                time_tot = time.time()-self.start_training_time
                
                # Print and save in txt file
                mystring_min = 'Epoch: {:d}, epoch time: {:.3f}min, tot time: {:.3f}day, L_train: {:.3f}, L_base: {:.3f}, L_test: {:.3f}, gap_train(%): {:.3f}, update: {}'.format(self.g_ep.value, time_one_epoch/60, time_tot/86400, mean_tour_length_train, mean_tour_length_baseline, mean_tour_length_test, 100*gap_train, update_baseline) 
                
#                 self.file.write(mystring_min+'\n')
                ...
                torch.save({
                    ...
                    'model_train':  self.gnet.state_dict(),
                    'optimizer': self.opt.state_dict(),
                    }, '{}.pkl'.format(checkpoint_dir + "/checkpoint_" + self.time_stamp + "-n{}".format(args.nb_nodes) + "-gpu{}".format(args.gpu_id)))
...
if __name__ == "__main__":
    ...
    gnet = TSP_net(args.dim_input_nodes, args.dim_emb, args.dim_ff, 
              args.nb_layers_encoder, args.nb_layers_decoder, args.nb_heads, args.max_len_PE,
    ...
    opt = SharedAdam(gnet.parameters(), lr=args.lr, betas=(0.92, 0.999))      # global optimizer
#     opt = torch.optim.Adam( gnet.parameters() , lr = args.lr ) 
    ...
    workers = [Worker(gnet, opt, global_ep, global_ep_r, res_queue, i,model_baseline,args,start_training_time,time_stamp) for i in range(4)]
    ...
    while True:
        ...
        if r is not None:
            ...
            for step in range(0,args.nb_batch_eval):
                ...
                # compute tour for model and baseline
                with torch.no_grad():
                    tour_train, _ = gnet(x, deterministic=True)
                    tour_baseline, _ = model_baseline(x, deterministic=True)
...
#             plot_performance_baseline.append([ (epoch+1), mean_tour_length_baseline])
            ...
            # Print and save in txt file
            mystring_min = 'Epoch: last, L_train: {:.3f}, L_base: {:.3f}, L_test: {:.3f}, gap_train(%): {:.3f}, update: {}'.format(
                 mean_tour_length_train, mean_tour_length_baseline, mean_tour_length_test, 100*gap_train, update_baseline) 
            print(mystring_min) # Comment if plot display
...
#                 }, '{}.pkl'.format(checkpoint_dir + "/checkpoint_" + time_stamp + "-n{}".format(args.nb_nodes) + "-gpu{}".format(args.gpu_id)))
    ...
    plt.xlabel('Step')
    plt.show()
    plt.savefig('./pic./meanlength'+time_stamp+'.png')  
